...
| 4 | [Hermes 长期英语学习班主任接入方案](./英语班主任.md) | 看 Hermes 如何作为长期学习画像、每日任务和周复盘 Provider 接入 PEAI。 |
| 5 | [写作教练 Agent 设计](./写作教练Agent设计.md) | 看从零到一作文陪写、学段/考试标准、偏题检查和多轮循环方案。 |
| 6 | [写作教练 Schema 设计](./写作教练Schema设计.md) | 看写作教练 input context、审题 output、提纲 output 和字段字典。 |
| 7 | [写作路由与 Patch 方案](./写作路由与Patch方案.md) | 看 WritingIntentPlanner、WritingPatch 和 Patch 应用器如何协作。 |
| 8 | [路由 Agent 设计](./路由Agent设计.md) | 看路由 Agent 的输入、输出、模型配置边界和工作流决策 schema。 |
| 9 | [Agent 可观测性与调试中心](./Agent可观测性与调试中心.md) | 看 Agent Debug Recorder、Eval Dataset Builder、DeepEval 和 Langfuse 接入方案。 |
| 10 | [AI 调试端设计](./AI调试端设计.md) | 看 `/ops/agent/*` 的页面结构、使用边界和数据接入顺序。 |
| 11 | [OpenAI Agents SDK 中文学习笔记](./OpenAI Agents SDK中文学习笔记.md) | 查官方 SDK 概念、术语翻译和 PEAI 代码用法映射。 |
| 12 | [Function Call 学习笔记](./FunctionCall学习笔记.md) | 学习 function/tool calling 的调用链、schema、工具边界和 PEAI 落地方式。 |
| 13 | [Structured Output 学习笔记](./StructuredOutput学习笔记.md) | 学习结构化输出、schema 约束、Agents SDK output_type 和 PEAI 输出契约设计。 |
| 14 | [Agent Builder 学习资料](./agent-builder/) | 学习 OpenAI Platform 可视化工作流、节点、变量和 PEAI 映射方式。 |
| 15 | [对话词句采集清洗方案](./数据清洗/对话词句采集清洗方案.md) | 看对话词句如何从 Agent 回复中采集、清洗、评分并落库。 |
...
| 技术设计 | 模块边界、数据流、schema、运行链路 | [学习助手 Agent 编排架构](./学习助手Agent编排架构.md)、[Hermes 长期英语学习班主任接入方案](./英语班主任.md)、[路由 Agent 设计](./路由Agent设计.md)、[写作教练 Agent 设计](./写作教练Agent设计.md)、[写作教练 Schema 设计](./写作教练Schema设计.md)、[写作路由与 Patch 方案](./写作路由与Patch方案.md) |
