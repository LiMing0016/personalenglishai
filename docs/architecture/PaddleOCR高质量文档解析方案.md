...
- Windows 后端不直接承担 Mac high_quality 能力，只通过 `APP_OCR_PADDLE_BASE_URL` 调用远程服务。
...
| `pages[].elements[].source` | `provider` + `metadata.source` | 区分文本、公式、未来表格 |
