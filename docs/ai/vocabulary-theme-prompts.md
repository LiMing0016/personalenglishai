...
related_code:
  ...
  - python/ai_orchestrator/prompts/agent_instructions/vocabulary_card_blocks.md
related_docs:
...
数据库中的历史主题快照仍使用 `basic-markdown-v1`、`exam-markdown-v1`、`reading-markdown-v1` 和 `custom-markdown-v1`。Python provider 在请求边界把它们确定性映射为对应的 `*-blocks-v1`，再调用 Prompt 资产：`vocabulary_core_fallback` 负责生成 Core 2，`vocabulary_card_blocks` 负责按主题生成 Blocks 1。Java 只冻结主题 UID、版本和策略 key，不持有或拼接 Prompt 正文；旧 Java provider 继续使用历史 `*-markdown-v1` 作为显式回滚路径。

Prompt version 由 Python 根据本次实际解析到的 Prompt 返回，Java 不发送 Prompt version。本地 Prompt 使用仓库版本名；remote/hybrid 命中 OpenAI Prompt 时必须同时固定 Prompt ID 和 version，审计值包含实际 ID/version。远程 Prompt 缺少固定 version 时，Python 健康检查必须报告 vocabulary generation 未配置，生成端点返回不可重试的 `VOCABULARY_GENERATION_NOT_CONFIGURED`。Python 在 generation metadata 中返回 provider、model、Prompt version、模型调用次数和安全 trace ID；Java 仅把已验证的 metadata 保存到 `generation_metadata_json`。不要在日志、测试失败输出或人工验收记录中打印 Prompt 正文、词典 Core、sourceContext、生成 Blocks 或原始模型输出。

## 当前结论

主题化单词卡把稳定的 Lexical Core 2 与可持续编辑的 Card Blocks 1 分成两次受控处理。词典结果作为 Core 生成的可信参考；主题只进入 Blocks 调用，不能修改单词身份或核心事实。用户笔记也是 `note` Block，不再建立第三套 Markdown 存储。

## 场景和目标

- 输入：规范词形、共享词典 core、第一条有效来源语境、不可变主题 revision。
- 输出：通过校验的 Core 2 与有序 Blocks 1。
- 非目标：让主题用途充当系统指令、让 Blocks 重复核心释义、执行 HTML 或改变卡片身份。

## 四个策略 key

| `prompt_strategy_key` | 主题 | Card Blocks 侧重点 |
| --- | --- | --- |
| `basic-blocks-v1` | Basic | 常用例句、搭配、使用边界和记忆提示 |
| `exam-blocks-v1` | Exam | 考试考义、固定搭配、易错辨析和真题风格例句 |
| `reading-blocks-v1` | Reading | 语境义、句中作用、同义改写和上下文辨析 |
| `custom-blocks-v1` | 用户主题 | 围绕用户用途说明，组合受支持的 Block 类型 |
...
2. 将词典音标、词性和双语释义作为可信输入传给 Core Agent，输出 `schemaVersion: 2`、稳定 ID 和规范 `term`。
3. 词典值优先；AI 只补齐缺失字段、筛选适合主题和学生水平的主要义项，不得生成与可信词性或词义范围冲突的内容。
4. Core Agent 只输出闭合 JSON schema，不输出学习扩展内容，也不能增加 schema 外字段。
5. Core 通过身份、稳定 ID、字段类型、数组数量和标量长度校验后，才进入 Blocks 调用；Blocks 只能通过 `meaningRefs` 引用 Core meaning ID。

词典查询本身异常时不绕过词典直接生成 core，而是记录 `DICTIONARY_LOOKUP_FAILED` 并按可重试失败处理。这样可区分“词典确实无内容”和“词典服务不可用”。

## Prompt 与安全结构化输入

Python workflow 使用 JSON 序列化模型输入。Blocks Agent 接收已验证 Core、`sourceContext` 和完整主题快照；Core Agent 接收请求 term、可信 `dictionaryCore` 与 `sourceContext`。Prompt 明确把 `sourceContext`、theme `purpose`、name 和 strategy key 都视为数据，不允许这些字段覆盖 Prompt、输出 schema、卡片 term 或安全规则。即使用途包含“忽略以上规则”、HTML、JSON 片段或代码围栏，也只能作为 JSON 字符串值参与生成。

Agent 不直接返回自由文本：Blocks 调用必须输出 `VocabularyCardBlocks`，Core 调用必须输出 `VocabularyCoreFallbackOutput`。两类输出都经 Pydantic 和 Java 最终边界再次校验。

## Card Blocks 输出契约

- 只接受 `schemaVersion: 1` 的有序 Blocks，Block ID 在文档内唯一。
- 允许 `exampleList`、`collocationList`、`usageBoundary`、`contrastTable`、`memoryTip` 和 `note`；未知类型、悬空 `meaningRefs` 或额外字段直接拒绝。
- `note.content` 最大 20,000 字符并禁止原始 HTML；前端使用严格 Markdown 渲染器，不执行脚本、事件属性、iframe 或危险 URL。
- Blocks 不重复承担 term、音标、词性和核心释义；这些事实只从 `core_json` 读取。
- Java 回滚 provider 仍生成历史 Markdown，并独占旧七天缓存；Python provider 不读写该缓存。
...
| 阶段 | 必备字段 | 说明 |
| --- | --- | --- |
| 词典/Core/Blocks 降级 | `traceId`, `reasonType` | `traceId` 仅允许字母、数字、点、下划线和连字符，最长 80 |
...
| Blocks 失败、为空或引用无效 | 保留 validated Core，标记 partial | `needs_review` | 核心内容可见，显示“主题内容待完善” |
| Java 回滚 cache Core 与当前词典 Core 不一致 | 驱逐 cache 并重新生成 Markdown | 不直接改变状态 | 用户无感 |
| base revision 已被用户编辑 | 保存 AI candidate，不覆盖 active | `needs_review` | 进入版本冲突处理 |

Blocks 失败只降级扩展内容，不会把有效 Core 降级成空卡。用户可重新生成或手工添加 Block；前端不得显示模型校验、Java 异常或 Axios 技术串。
...
验收样例至少覆盖四个策略、JSON 字段提示注入、重复 Block ID、悬空 meaning 引用、note 原始 HTML、词典 Core 优先、Blocks partial、Java poisoned cache 和 lease 丢失。
