...
- 最近 12 条非空消息优先进入路由上下文候选。
- 在剩余预算允许时继续向前补充，最多保留 30 条消息，约 15 轮对话。
- 全部路由历史总预算约 20000 字符。
- 单条消息最多约 2000 字符。
- 短消息完整保留。
- 超长消息保留开头约 700 字符和结尾约 1300 字符，中间使用明确省略标记。
- 从最新消息向前分配总预算，优先保留最近上下文；更早消息只使用剩余预算。
- 当前用户消息和明确 interaction 不计入历史裁剪候选，必须完整进入 RouteRequest。
...
- 最近 12 条消息优先保留，并在预算允许时向前补充到最多 30 条。
- 路由历史不超过总预算。
- 总预算约 20000 字符，单条消息预算约 2000 字符。
- 超长助手回复按约 700/1300 的比例同时保留开头和结尾。
- interaction 字段完整映射。
- 空消息不会占用历史预算。
