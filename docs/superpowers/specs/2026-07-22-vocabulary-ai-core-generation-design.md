...
- 初次生成由 AI 根据主题提供 2 至 3 个高频或语境相关词义，并生成中英文成对释义。
...
- `term`、可信音标、真实音频和基础词性属于稳定词汇事实。
- 重点释义、例句、搭配、语境解释和个人笔记属于用户学习内容。
- AI 可以起草或建议用户学习内容，只有初次明确沉淀或后续显式添加后才成为卡片资产。
- 用户可以编辑、删除和调整 AI 生成的内容。
...
首版支持 `addMeaning`、`addExample`、`addCollocation` 和 `addNote`。后端负责应用命令、校验边界和创建 revision；模型不能直接执行数据库写入。
...
- AI 根据主题选择适合初次学习的主要词义，不机械复制词典全部释义。
...
1. **AI 基础卡片**：让完整 Oxford 输入也经过 AI Core Agent，生成 2 至 3 个重点词义，并完成失败降级和元数据改造。
2. **增量写入能力**：增加有限的增量命令、唯一主卡查询、精确去重、幂等和 revision 冲突保护；先通过后端测试验证，不依赖学习助手页面。
3. **学习助手接入**：在对话中展示已有卡片摘要和本次增量，只有用户点击“添加到单词卡”才调用写入能力。
