...
首轮在完成统一 AI 导入主链路的同时，优先落实以下三项工程约束：

- 请求竞态：取消旧分析，并保证只有最新请求可以更新候选词。
- 性能与超时：沿用明确的前端、Java 和 Python 超时预算与用户可见状态。
- 结果一致性：分析结果必须绑定输入指纹，输入改变后旧结果立即失效。

移动端拍照入口、额外隐私加固和统一分析新埋点暂不纳入首轮；现有文件校验、敏感内容不写普通日志和旧产品事件行为保持不退化。
...
- `inputFingerprint`：必填，浏览器根据当前文本和图片计算的 SHA-256 小写十六进制值。
...
前端统一分析请求使用 `AbortController` 和单调递增的本地 `requestId`：

1. 发起新分析前取消当前控制器，并递增 `requestId`。
2. 请求完成后同时校验未被取消、响应对应最新 `requestId`，再写入候选词。
3. 用户修改文本、替换图片、移除图片或卸载组件时取消当前请求并递增 `requestId`。
4. 已取消请求的异常不显示为失败提示，迟到成功响应也不能覆盖当前状态。

“AI 分析”请求进行中按钮禁用，避免同一次输入重复扣费。Java 和 Python 继续使用业务 `traceId` 关联日志，但浏览器最新请求判断不依赖服务端返回顺序。

## 性能与超时预算

统一分析使用同步请求，不新增后台任务或轮询：

- 产品目标：P50 不超过 8 秒，P95 不超过 20 秒。
- 浏览器 Axios 硬超时：60 秒。
- Java 到 Python HTTP 超时：55 秒。
- Python workflow 总预算：45 秒，包含初始结构化调用和最多一次结构重试。

分析期间按钮显示“分析中…”，并通过 `aria-live` 告知状态。超时、服务不可用或结构化失败时保留输入和图片，允许用户显式重试；不执行无限重试或自动再次扣费。

## 输入指纹与结果一致性

前端对每次分析输入计算 `inputFingerprint`。算法固定为 SHA-256：先对文本执行换行统一、去除首尾空白，再编码为 UTF-8；随后拼接一个零字节和图片原始字节，缺少图片时只拼接零字节。最终编码为小写十六进制字符串。

公开分析请求携带 `inputFingerprint`。Java 使用收到的文本和文件按同一算法重新计算；不一致时返回 400 稳定错误 `VOCABULARY_IMPORT_FINGERPRINT_MISMATCH`，且不调用 Python。成功响应原样返回 Java 校验后的 `inputFingerprint`。

前端保存“最后成功分析指纹”。文本或图片发生变化后重新计算当前指纹；与最后成功指纹不一致时：

- 保留旧候选供用户参考，但显示“输入已变化，请重新分析”。
- 禁用“生成卡片”。
- 禁止把旧分析 trace 和当前输入混合提交。

只有最新请求的响应指纹、请求开始时指纹和当前输入指纹三者完全相同时，候选词才标记为可提交。
