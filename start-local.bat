...
start "PEAI Backend %BACKEND_PORT%" powershell -NoExit -ExecutionPolicy Bypass -Command "$env:SERVER_PORT='%BACKEND_PORT%'; $env:APP_BASE_URL='http://localhost:%WEB_PORT%'; $env:ASSISTANT_ORCHESTRATOR_BASE_URL='http://%PYTHON_HOST%:%PYTHON_PORT%'; $env:VITE_ASSISTANT_API_BASE_URL='http://%PYTHON_HOST%:%PYTHON_PORT%'; $env:AI_CONTEXT_CONVERSATION_PYTHON_BASE_URL='http://%PYTHON_HOST%:%PYTHON_PORT%'; Set-Location -LiteralPath '%BACKEND_DIR%'; .\mvnw.cmd spring-boot:run"
