...
test('card detail uses a compact accessible back icon', () => {
  assert.match(inspector, /import\s*\{[^}]*ArrowLeft[^}]*\}\s*from\s*'lucide-vue-next'/)
...
test('card detail header provides pronunciation and continuous navigation controls', () => {
  assert.match(inspector, /import\s*\{[^}]*ChevronLeft[^}]*ChevronRight[^}]*LoaderCircle[^}]*Volume2[^}]*\}\s*from\s*'lucide-vue-next'/s)
  assert.match(inspector, /useVocabularyPronunciation/)
  assert.match(inspector, /import type\s*\{\s*VocabularyCardSequence\s*\}/)
  assert.match(inspector, /navigation\?:\s*VocabularyCardSequence\s*\|\s*null/)
  assert.match(inspector, /navigate:\s*\[direction:\s*'previous'\s*\|\s*'next'\]/)
  assert.match(inspector, /class="card-inspector__term-button"/)
  assert.match(inspector, /:aria-label="`播放 \$\{card\.displayTerm\} 的默认发音`"/)
  assert.match(inspector, /class="card-inspector__pronunciation-button"/)
  assert.match(inspector, /<Volume2[^>]*aria-hidden="true"/)
  assert.match(inspector, /<LoaderCircle[^>]*aria-hidden="true"/)
  assert.match(inspector, /@pronounce="playPhonetic"/)
  assert.match(inspector, /class="card-inspector__sequence"/)
  assert.match(inspector, /navigation\.position/)
  assert.match(inspector, /navigation\.total/)
  assert.match(inspector, /navigationDisabled/)
  assert.match(inspector, /emit\('navigate', direction\)/)
  assert.match(inspector, /aria-live="polite"/)
})
