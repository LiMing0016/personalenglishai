...
assert.ok(!appLayoutSource.includes(":deep(.app-rail)"), 'writing route should let the shared rail inherit the page surface')
assert.ok(appLayoutSource.includes('--app-sidebar-border: #e4dfd3'), 'writing route should keep the warm sidebar divider')
