<template>
  <section class="vocabulary-card-list" aria-label="沉淀单词卡列表">
    ...
    <div v-else class="card-list-rows" role="list">
      ...
      >
        <span class="card-term"><strong>{{ card.displayTerm }}</strong><small>{{ card.templateKey }}</small></span>
        <span class="card-source">{{ card.sourceTypes.join(' / ') || '未标注来源' }}</span>
        <span class="card-status" :class="`card-status--${card.status}`">{{ statusLabel(card.status) }}</span>
      </button>
    </div>
...
watch([keyword, status, sourceType], () => emitFilters(1))
...
.card-list-filters { display: grid; grid-template-columns: minmax(180px, 1fr) 132px 132px; gap: 10px; padding: 14px 0; }
...
.card-list-rows { border-top: 1px solid #edf2f7; }.card-row { display: grid; grid-template-columns: minmax(150px, 1.2fr) minmax(140px, 1fr) 92px; width: 100%; min-height: 68px; align-items: center; gap: 14px; border: 0; border-bottom: 1px solid #edf2f7; background: transparent; color: #334155; font: inherit; padding: 0 12px; text-align: left; cursor: pointer; }.card-row:hover, .card-row.selected { background: #f0fdf4; }.card-term { min-width: 0; }.card-term strong, .card-term small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.card-term strong { color: #0f172a; }.card-term small, .card-source { color: #64748b; font-size: 12px; }.card-source { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.card-status { justify-self: start; border-radius: 999px; font-size: 12px; font-weight: 800; padding: 5px 8px; }.card-status--generating { background: #dbeafe; color: #2563eb; }.card-status--ready { background: #dcfce7; color: #047857; }.card-status--needs_review { background: #fef3c7; color: #b45309; }.card-status--failed { background: #fee2e2; color: #b91c1c; }.card-status--captured { background: #e2e8f0; color: #475569; }
